...
layui.use(['form','upload'], function(){
    ...
    upload.render({
        elem: '#multiple'
        ,url: "http://localhost:8088/api/v1/qiniu/uploadImg"
        ,accept: 'images'
        ,acceptMime: 'image/*'
        ,size:102400
        ,drag:true
        ...
        ,done: function(res){
            ...
            $("#img-thumb a").attr('href',res.data);
                 $("#img-thumb img").attr('src',res.data);
                 $("#url").val(res.data);
                 $("#html").val("<img src = '" + res.data + "' />");
                 $("#markdown").val("![](" + res.data + ")");
                 $("#bbcode").val("[img]" + res.data + "[/img]");
                 $("#imgshow").show();
             }
             layer.closeAll('loading');
        }
...
$("#pay").click(function () {
    ...
    layer.open({
        ...
        btnAlign: 'c',
        moveType: 1,
        content: "https://www.yanxiaolong.cn/#/home",
        btn:['残忍拒绝'],
        yes:function () {
            layer.closeAll();
        }
